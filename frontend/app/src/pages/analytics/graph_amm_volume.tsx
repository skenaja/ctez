...
const GraphAMMVolume: React.FC = () => {
    ...
    const { data: data1m = false } = useCtezGraphAMMVolume() as { data: OneLineGraph[] | false };
    const { data: dataAll = false } = useCtezGraphAMMVolumeAll() as { data: OneLineGraph[] | false };
            ...
            <Flex justifyContent='space-between'>
                <div>
                    ...
                    <Flex flexDirection='column'>
                        ...
                        >
                            {(data1m && !value)
                                ? `$${numberToMillionOrBillionFormate(data1m[data1m.length - 1].value)}`
                                : value
                                    ? `$${numberToMillionOrBillionFormate(value)}`
                                    : <SkeletonText pr={6} noOfLines={1} spacing="1" />}
                        </Text>
                        {time
                            ? <Text fontSize='12px'>{activeTab === '1m' ? dateFormat(time) : dateFormat2(time)}</Text>
                            : <Text fontSize='12px' opacity={0}>Time</Text>
                        }
                    </Flex>
                </div>
                <ButtonGroup variant='ghost' gridGap={2} textColor={textcolor} fontSize='12px' spacing='-1'>
                    <Button
                        fontSize='12px'
                        className={activeTab === '1m' ? "btnactive" : ''}
                    ...
                    <Button
                        fontSize='12px'
                        className={activeTab === 'all' ? "btnactive" : ''}
                        textDecoration='underline'
